...
export function parseHours(hoursStr: string): ParsedHours {
  ...
  if (multiTimeMatch) {
    ...
    if (start1H && start1M && end1H && end1M && start2H && start2M && end2H && end2M) {
      return {
        ...
        periods: [
          {
            start: timeToNumber(parseInt(start1H), parseInt(start1M)),
            end: timeToNumber(parseInt(end1H), parseInt(end1M)),
          },
          {
            start: timeToNumber(parseInt(start2H), parseInt(start2M)),
            end: timeToNumber(parseInt(end2H), parseInt(end2M)),
          },
        ],
      };
    }
  ...
  if (timeMatch) {
    ...
    if (startH && endH) {
      return {
        type: "normal",
        start: timeToNumber(parseInt(startH), parseInt(startM)),
        end: timeToNumber(parseInt(endH), parseInt(endM)),
      };
    }
  }
  ...
  if (simpleTimeMatch) {
    ...
    if (startH && endH) {
      return {
        type: "normal",
        start: timeToNumber(parseInt(startH), 0),
        end: timeToNumber(parseInt(endH), 0),
      };
    }
  }
...
export function formatBusinessHours(businessHours: Record<string, string>) {
  ...
  return {
    isOpen,
    currentStatus: getStatusText(parsedToday, isOpen, todayHours),
    todayHours,
  };
}
